...
def whipMake():
    ...
    if pm.objExists('whipContr_1'):
        pm.confirmDialog(title = u'鞭リグツール', message = u'鞭リグ既に存在します', button = ['OK'])
        ...
        ikStartJ = pm.ls('Joint_Weapon2')[0]
        '''
        counter = 18
        ...
        clusterGroup = pm.group(name = 'clusterGroup')
        contrGroup = pm.group(name = 'controlGroup')
        rad = 14
        for i, cv in enumerate(curveCVs): #building clusters on each curve
            
            clust = pm.cluster(cv, name = 'whipCluster' + str(i), relative = True)
            ...
            pm.parent(clust[1], clusterGroup)
            pm.select(deselect = True)
            pm.parent(circ[0], contrGroup)
            rad -= 1
        pm.parent(clusterGroup, ik_return[0], 'Joint_Weapon')
        #pm.parentConstraint('Joint_Weapon', 'whipContr_0', mo = True)
        
    else:
        pm.confirmDialog(title = u'鞭リグツール', message = u'シーンに鞭が見つけていません', button = ['OK'])