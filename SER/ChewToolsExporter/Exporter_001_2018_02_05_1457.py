...
class MainWindow(QtWidgets.QDialog, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        if self.fileName[2] == '_':#to check if it is a common weapon motion
            ...
            self.boneFBX = self.weaponPathIndex[self.fileName[:2]] #assigning weapon export path as export path
            pathRead.close()
            
            
        else: #file is character motion
        ...
        #defining chara name
        if self.fileName[2] == '_':
            self.charaNameText = self.weaponIndex[self.fileName.split('_')[0]] + '　共通モーション'
        else:
            try:
                self.charaNameText = self.charaNameIndex[self.fileName[:3]]
            except:
                pass
        ...
        if self.fileName[2] == '_': #weapon common motion
            self.exportPathTex = self.boneFBX
        else: #motion is character motion
            try:
                self.exportPathTex = self.boneFBX + '/' + self.fileName[:3] + '/Motions/' + self.weaponIndex[self.fileName.split('_')[1]]
            except:
                pass
        ...
        self.exportPath.setPlainText(self.exportPathTex)
        ...
        self.animExportButton.clicked.connect(self.animExpButton)#initialising button for file open
    ...
    def modelExport_2:
        ...
        cmds.file("E:/SER working folder/test/Unity Folder/MODEL/testMesh1.fbx", force = True, type = 'FBX export', exportAll = True, options = 'v=0')#temporary, change address later
