...
class PoseLib():
    ...
    def __abs__(self):
        self.indexPath = r'\\p.sv\Tool\PrismTools\NAVY\PoseLib\index_motion.index'
    ...
    def poseTest():
        ...
        def animSave(_):
            ...
            for frame in range(int(animAPI.MAnimControl.maxTime().value() + 1)):
                ...
                line = ''
                for i in pm.listRelatives(pm.ls(sl=True), ad=True,
                                          type='joint'):  # remove ", type = 'joint'" for control rig version
                    if str(pm.listRelatives(i, parent=True)) == "[nt.Joint(u'Character_Head')]" or str(
                            pm.listRelatives(i, parent=True)) == "[nt.Joint(u'Helper_Weapon2')]" or str(
                            pm.listRelatives(i, parent=True)) == "[nt.Joint(u'Helper_Weapon1')]" or i[:5] == 'Joint':
                        continue
                    if pm.objectType(i) == "locator":
                        continue
                    line = line + str(int(pm.currentTime())) + ' ' + i + ' ' + str(
                        pm.xform(i, q=True, translation=True)) + ' ' + str(pm.xform(i, q=True, rotation=True)) + ' ' + str(
        ...
        poseFrame = pm.frameLayout(annotation='annotation test', label=u'ポーズ', labelIndent=5, width=590,
                                   parent='columnLayout01', marginHeight=5)
        pm.rowLayout('row1', nc=5, width=400)
        handSliderGrp = pm.intSliderGrp(field=True, label=u'手ポーズ', minValue=0, maxValue=handUpperLimit - 1, value=0, adj=3,
                                        columnWidth3=[100, 50, 200], width=525, dragCommand=setHandPose)
        # buttons for 2nd row
        pm.rowLayout('row2', nc=2, parent='columnLayout01')
        bodySliderGrp = pm.intSliderGrp(field=True, label=u'全身ポーズ', minValue=0, maxValue=upperLimit - 1, value=0, adj=3,
                                        columnWidth3=[100, 50, 200], width=525, dragCommand=setPose)
        ...
        resetButton = pm.button('resetButton', label=u'T　ポーズに', width=390, height=20, backgroundColor=(0.6, 0.6, 0.6),
                                parent='col_2', command=resetChar)
        ...
        resetButton_ctrlRig = pm.button('ctrlRigReset', label=u'Control Rig T pose', width=390, height=20,
                                        backgroundColor=(0.6, 0.6, 0.6), parent='col_4', command=resetCharTPose)
        ...
        resetButton_ctrlRig = pm.button('taisou', label=u'体操モーション', width=390, height=20, backgroundColor=(0.6, 0.6, 0.6),
                                        parent=col_6, command=animLoad)
