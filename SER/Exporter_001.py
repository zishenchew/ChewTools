...
class MainWindow(QtWidgets.QDialog, Ui_MainWindow):
    ...
    def __init__(self, fujiArg, parent=None):
        ...
        if self.fileNameSplit[0] == 'SER':  # charaModel
            ...
            self.charaNumber = '0'
            self.weaponNumber = '0'
        elif len(self.fileNameSplit[0]) == 2 and self.fileNameSplit[1] == 'CutScene' and self.fileNameSplit[
            2] == 'Chain':
            self.fileType = 'chain'
            self.charaNumber = '0'
            self.weaponNumber = self.fileNameSplit[0]
        elif len(self.fileNameSplit[0]) == 2 and self.fileNameSplit[1] != 'Resonize' and self.fileNameSplit[
            1] != 'CutScene':
        ...
        elif len(self.fileNameSplit[0]) == 3 and self.fileNameSplit[0] != 'SER' and self.fileNameSplit[0] != 'Gun' and \
                self.fileNameSplit[0] != 'Bow':
        ...
        if self.fileType == 'commonMotion':  # this weapon path is only needed if the file is a common motion
            ...
            self.commonMotionExportPath = self.weaponPathIndex[
                self.fileName[:2]]  # assigning weapon export path as export path
            pathRead.close()
        ...
        if self.fileType == 'charaMotion':
            self.exportPathText = self.boneFBX + '/' + self.charaNumber + '/Motions/' + self.weaponIndex[
                self.weaponNumber]
        elif self.fileType == 'kyojinMotion':
            self.exportPathText = self.boneFBX + '/' + self.charaNumber + '/Motions/' + self.weaponIndex[
                self.weaponNumber].lstrip('re_')
            ...
            self.exportPathText = 'D:/SER/SVN/MAYA/model/' + 'SER_0' + self.charaNumber[1:] + '_' + self.fileNameSplit[
                2] + '/FBX'
        elif self.fileType == 'cutScene':
            self.exportPathText = self.boneFBX + '/' + self.charaNumber + '/Motions/' + self.weaponIndex[
                self.weaponNumber]
            ...
            if self.fuji == 1:
                self.exportPathText = r'D:/SER/SER_SVN/Unity/motion/fbx_yard/Assets/Models_Characters/_Animations/' + \
                                      self.weaponIndex[self.weaponNumber]
                ...
                self.exportPathText = r'D:/SER/SVN/Unity/motion/fbx_yard/Assets/Models_Characters/_Animations/' + \
                                      self.weaponIndex[self.weaponNumber]
                if self.weaponNumber == '00':
                    self.exportPathText = r'D:/SER/SER_SVN/Unity/motion/fbx_yard/Assets/Models_Characters/_Animations/00_Common'
        elif self.fileType == 'kyojinIntro':
            self.exportPathText = r'D:/SER/SVN/Unity/motion/fbx_yard/Assets/Models_Characters/%s/Motions/Resonize' % \
                                  self.fileNameSplit[0]
        elif self.fileType == 'kyojinCommonStep':
            self.exportPathText = r'D:/SER/SVN/Unity/motion/fbx_yard/Assets/Models_Characters/_Animations/00_Common'
        elif self.fileType == 'kyojinStep':
            self.exportPathText = self.boneFBX + '/' + self.charaNumber + '/Motions/' + self.weaponIndex[
                self.weaponNumber].lstrip('00_')
        elif self.fileType == 'kyojinCommonFinish':
            self.exportPathText = r'D:/SER/SVN/Unity/motion/fbx_yard/Assets/Models_Characters/_Animations/00_Common'
        elif self.fileType == 'weaponModel':
            self.exportPathText = 'D:/SER/SVN/MAYA/model/' + pm.sceneName().rstrip('.mb').rstrip('.ma').split('/')[
                -3] + '/FBX'
        '''
        INPUTS ########################################################################################################################################################
        '''
        ...
        if enabled:
            if self.fileType == 'charaMotion':
                self.exportPath.setText(
                    r'D:/SER/SVN/Unity/motion/fbx_yard/Assets/CutScenes/CutScene_Special/' + self.fileNameSplit[
                        0] + '/Special_Camera')
            elif self.fileType == 'cutScene':
                self.exportPath.setText(
                    r'D:/SER/SVN/Unity/motion/fbx_yard/Assets/CutScenes/CutScene_Special/' + self.fileNameSplit[
                ...
                self.exportPath.setText(
                    r'D:/SER/SVN/Unity/motion/fbx_yard/Assets/CutScenes/CutScene_Resonize_03_hit/' + self.fileNameSplit[
                        0])
            elif self.fileType == 'kyojinMotion' and self.fileNameSplit[
                1] == 'ResonizeStep':  # resonize step  path adjustment
                ...
                self.exportPath.setText(
                    r'D:/SER/SVN/Unity/motion/fbx_yard/Assets/CutScenes/CutScene_Resonize_01_intro/' +
                    self.fileNameSplit[0])
            elif self.fileType == 'kyojinCommonFinish' and self.fileNameSplit[1] == 'ResonizeFinish':  # resonize finish
                self.exportPath.setText(
                    r'D:/SER/SVN/Unity/motion/fbx_yard/Assets/CutScenes/CutScene_Resonize_04_finish')
    ...
    def cameraExport(self):
        ...
        if pm.objExists('camera1_group') == False and pm.objExists('camera_group') == False and pm.objExists(
                'camera2') == True:  # creating a conditional for when the animator prefers to use a no-aim camera
            print('no aim')
    ...
    def aimCamMake(self):  # will be deprecated
        ...
        aimCam = pm.camera(coi=5, fl=35, lsr=1, cs=1, hfa=1.41732, hfo=0, vfa=0.94488, vfo=0, ff='Fill', ovr=1, mb=0,
                           sa=144, ncp=0.1, ow=30, pze=False, hpn=0, zoom=1)
        pm.rename(aimCam[0], 'camera1')
        mel.eval('cameraMakeNode 2 "";')  # creates camera with aim
        constr = pm.parentConstraint('camera2', 'camera1_group',
                                     mo=False)  # constraining the aimCam group to old camera
        ...
        pm.bakeResults(camLoc, aimLoc, simulation=True, time=(animAPI.MAnimControl.minTime().value(),
                                                              animAPI.MAnimControl.maxTime().value()))  # baking the movement into the camera
        ...
        pm.copyKey('cameraShape2',
                   time=(animAPI.MAnimControl.minTime().value(), animAPI.MAnimControl.maxTime().value()),
                   option='curve')  # copying all camera focal length and stuff
        try:
            pm.pasteKey('cameraShape1')  # pasting
        except:
            print('camShape has no keys')
        pm.setAttr('cameraShape1.filmFit', 2)
        pm.copyKey('camera2', time=(animAPI.MAnimControl.minTime().value(), animAPI.MAnimControl.maxTime().value()),
                   option='curve', at='rz')  # copying rotate/roll data
    ...
    def camConstraint(self):  # making no-aim camera
        ...
        pm.camera(newCamShape, edit=True, fl=pm.camera('cameraShape1', q=True, fl=True),
                  coi=pm.camera('cameraShape1', q=True, coi=True))  # adjusting attributes of camera shape
        pm.setAttr(newCamShape.filmFit, 2)
        pm.copyKey('cameraShape1',
                   time=(animAPI.MAnimControl.minTime().value(), animAPI.MAnimControl.maxTime().value()),
                   option='curve')  # copying of camera shape keys
        try:
            pm.pasteKey(newCamShape)  # pasting
        except:
            print('camShape has no keys')
        pm.parentConstraint('camera1', newCamGrp[0], mo=False)  # constraining camera
        pm.bakeResults(newCamGrp[0], simulation=True, time=(animAPI.MAnimControl.minTime().value(),
                                                            animAPI.MAnimControl.maxTime().value()))  # baking the movement into the camera
        pm.keyframe(newCamGrp[0].rotateZ, edit=True, animation='objects', relative=True,
                    valueChange=pm.camera('cameraShape1', q=True,
                                          filmRollValue=True))  # adding the roll to the camera rotateZ
        pm.select(newCamGrp)
    ...
    def animExpButton(self):
        try:
            ...
            pm.renameFile(
                self.saveFileName.replace('_temp', ''))  # renames the file back to the original name before it was
            ...
            if self.camExport.isChecked() == True:  # camera export
                copy2(self.exportPath.text() + r'/' + self.exportName.text() + '_cam.fbx',
                      r'D:/SER/GIT/Assets/AssetBundle/Resources' + self.exportPath.text()[
                                                                   39:])  # r'D:\SER\GIT/Assets/AssetBundle/Resources' +
                print self.exportPath.text() + r'/' + self.exportName.text() + '_cam.fbx', r'D:/SER/GIT/Assets/AssetBundle/Resources' + self.exportPath.text()[
                                                                                                                                        39:]
            else:
                copy2(self.exportPath.text() + r'/' + self.exportName.text() + '.fbx',
                      r'D:/SER/GIT/Assets/AssetBundle/Resources' + self.exportPath.text()[
                                                                   39:])  # r'D:\SER\GIT/Assets/AssetBundle/Resources' +
                print self.exportPath.text() + r'/' + self.exportName.text() + '.fbx', r'D:/SER/GIT/Assets/AssetBundle/Resources' + self.exportPath.text()[
                                                                                                                                    39:]
    ...
    def animExport_1(self):
        ...
        if self.fileType == 'charaMotion' and self.fileNameSplit[2] != 'Special' or self.fileType == 'commonMotion' or \
                self.fileNameSplit[1] == 'ResonizeIdle':
            pass
        else:
    ...
    def animExport_2(self):
        ...
        pm.tabLayout('gameExporterTabLayout', st='gameExporterAnimationTab',
                     edit=True)  # switches over to the animation tab
        mel.eval(
            'gameExp_CurrentTabChanged();gameExp_UpdatePrefix;gameExp_PopulatePresetList();gameExp_CreateExportTypeUIComponents();')  # updates the window
        ...
        mel.eval(
            'setAttr($gGameFbxExporterCurrentNode + ".exportPath") - type "string" "' + self.exportPath.text() + '";')  # setting address

        formLayout1 = pm.layout('anim_gameExporterExportTypeFormLayout', query=True, childArray=True)[
            0]  # formLayout1, formLayout2, fieldText1 and self.clipNameFieldpy are all
        ...
        if self.ingameExport.isChecked() == True:  # motion export
            mel.eval(
                'gameExp_SetUniqueAnimationClipName 0"' + self.exportName.text() + '"' + self.clipNameFieldpy + ';')  # changing name? the 0 at the start indicates it position in the list of game clips
            pm.select('Character_Holder')
        elif self.camExport.isChecked() == True:  # aim camera export
            mel.eval(
                'gameExp_SetUniqueAnimationClipName 0"' + self.exportName.text() + '_cam' + '"' + self.clipNameFieldpy + ';')  # changing name? the 0 at the start indicates it position in the list of game clips
    ...
    def bakeHIK(self):
        ...
        if pm.ls('Controller_Weapon_Global_R') and pm.ls(
                'Controller_Weapon_Global_L'):  # baking and constraints for dual swords etc
            pm.bakeResults(pm.ls('*Joint_Weapon')[0], pm.ls('*:Joint_Weapon')[0],
            ...
            helperRToWeaponR = pm.parentConstraint(pm.ls('*Joint_Weapon', type='joint')[0], pm.ls('*Helper_Weapon1')[0],
                                                   mo=False)
            helperLToWeaponL = pm.parentConstraint(pm.ls('*:Joint_Weapon', type='joint')[0],
                                                   pm.ls('*Helper_Weapon2')[0], mo=False)
            pm.bakeResults(pm.ls('*Helper_Weapon1')[0], pm.ls('*Helper_Weapon2')[0],
                           t=(animAPI.MAnimControl.minTime().value(), animAPI.MAnimControl.maxTime().value()),
                           simulation=True)
            ...
            pm.bakeResults(pm.ls('*Joint_Weapon')[0],
                           t=(animAPI.MAnimControl.minTime().value(), animAPI.MAnimControl.maxTime().value()),
                           simulation=True)
            helperToWeapon = pm.parentConstraint(pm.ls('*Joint_Weapon', type='joint')[0], pm.ls('*Helper_Weapon1')[0],
                                                 mo=False)
    ...
    def helperShadowBake(self):
        pm.bakeResults('Helper_Shadow', simulation=True,
                       time=(animAPI.MAnimControl.minTime().value(), animAPI.MAnimControl.maxTime().value()),
    ...
    def deleteNonHIK(self):
        ...
        for i in pm.ls(type='transform'):
            if not pm.listRelatives(i,
                                    parent=True) and not i == 'top' and not i == 'side' and not i == 'persp' and not i == 'front' and not i == 'Character_Holder':
                delList.append(i)
        pm.delete(delList)
    ...
    def modelExport_2(self):
        # exporting
        if self.fileType == 'charaModel':
            cmds.file(self.exportPathInput_2.text() + '/' + self.charaNumber + self.kyojinka + 'Model.fbx', force=True,
                      type='FBX export', exportAll=True,
                      options='v=0')  # underscore is already included in the kyojinka string on both sides
        elif self.fileType == 'weaponModel':
            cmds.file(self.exportPathInput_2.text() + '/' + pm.sceneName().rstrip('.mb').rstrip('.ma').split('/')[
                -1] + '.fbx', force=True, type='FBX export', exportAll=True,
                      options='v=0')  # underscore is already included in the kyojinka string on both sides
