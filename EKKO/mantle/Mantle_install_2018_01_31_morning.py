...
def mantleCreate(charaHeight = 2): #default 2, 160cm height
    ...
    for i in locList:
        ...
        if i[2:5] == 'cR':
            pm.transformLimits(j, rotationX = [-5, 90], rotationY = [-45, 45], rotationZ = [-45, 25], erx = [True, True], ery = [True, True], erz = [True, True])
        elif i[2:5] == 'cM':
            pm.transformLimits(j, rotationX = [-5, 90], rotationY = [-45, 45], rotationZ = [-15, 15], erx = [True, True], ery = [True, True], erz = [True, True])
        elif i[2:5] == 'cL':
            pm.transformLimits(j, rotationX = [-5, 90], rotationY = [-45, 45], rotationZ = [-25, 45], erx = [True, True], ery = [True, True], erz = [True, True])
    ...
    pm.delete(locList, locSimList, locDict) #deleting unnecessary stuff used to create the rig
    ...
    if charaHeight == 1:#importing mantle Mesh
        ...
        cmds.file('K:/design/maya/data/tool/scData/Mant/mantMesh/mantMesh170.ma', i = True)#adjust address later
    elif charaHeight == 4:
        cmds.file('K:/design/maya/data/tool/scData/Mant/mantMesh/mantMesh180.ma', i = True)#adjust address later
