...
def sleeveCreate(charaSize = 2, insertKey = 0):
    
    curTime = pm.currentTime(query = True)
    
    if insertKey == 1: #insertKey 1 sets character to T-Pose at -100f, 0 doesn't
        pm.currentTime(-100, edit = True)
        mel.eval('source kkCharaSetup;\nkkCharaSetup();\nkkCharaBindPose_ex body;')
    ...
    if charaSize == 0:
        ...
        if charaSize == 1:
            cmds.file('K:/design/maya/data/tool/scData/Sleeve_Install/sleeve_mesh_150.ma', i = True) #
            upperArmDist = 10
        if charaSize == 2:
            cmds.file('K:/design/maya/data/tool/scData/Sleeve_Install/sleeve_mesh_160.ma', i = True) #
            upperArmDist = 11.613
        if charaSize == 3:
            cmds.file('K:/design/maya/data/tool/scData/Sleeve_Install/sleeve_mesh_170.ma', i = True) #
            upperArmDist = 14.5
        if charaSize == 4:
            cmds.file('K:/design/maya/data/tool/scData/Sleeve_Install/sleeve_mesh_180.ma', i = True) #
            upperArmDist = 17.5
    ...
    if charaSize !=0:
        ...
        if charaSize == 3:
            pm.xform('L_sleeve_mesh', translation = [0, -0.752686, 0], relative = True)
            pm.xform('R_sleeve_mesh', translation = [0, -0.752686, 0], relative = True)
        if charaSize == 4:
            pm.xform('L_sleeve_mesh', translation = [0, -0.75, 0], relative = True)
            pm.xform('R_sleeve_mesh', translation = [0, -0.75, 0], relative = True)
    ...
    for i in allList:
        ...
        if i.find('L_upperarm2') != -1:
            pm.move(upperArmDist, 0, 0, j, relative = True) #translate x to upperArmDist
        ...
        if jointDupe[0].find('L_UpperArm') != -1:
            pm.move(upperArmDist - 1.5, 0, 0, jointDupe, relative = True) #translate x to upperArmDist, it is slightly offset to fix the sleeve pop
        if jointDupe[0].find('R_UpperArm') != -1:
            pm.move(-upperArmDist + 1.5, 0, 0, jointDupe, relative = True) #translate x to -upperArmDist
    ...
    if pm.objExists('Poly'):
        parentGroup = 'Poly'
    elif pm.objExists('poly'):
        parentGroup = 'poly'
    else:
        parentGroup = 'CharaA'
    
    pm.parent('R_sleeve_mesh', 'L_sleeve_mesh', parentGroup)
    ...
    if insertKey == 1:#delecting key on frame -100
    ...
    pm.currentTime(curTime, edit = True)
    print('EKKO sleeve Install complete!')